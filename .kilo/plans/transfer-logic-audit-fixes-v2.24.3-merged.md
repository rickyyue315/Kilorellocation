...
## 新增發現（2025-06-11 獨立審核，對比後合併）

### NEW-A E2 Phase 2 跨 OM 允許雙重角色
- 檔案：strategies/e2_mode.py:113
- 問題：Phase 2 呼叫 `validate_pair(..., check_source_in_receive_sites=False)`，Phase 1 已接收的店可在 Phase 2 作為轉出源。
- 修正：改為 `check_source_in_receive_sites=True`。

### NEW-B E2 Phase 3 無視用戶設定的 c1_ceiling
- 檔案：strategies/e2_mode.py:204
- 問題：Phase 3 使用硬編碼 `C_MODE_ABS_CAP = 3`，用戶 UI 設定的 c1_ceiling 被忽略。
- 修正：傳入 logic.c1_ceiling 作為上限值。

### NEW-C F 模式 gap-fill 缺失 validate_pair + apply_transfer（擴充 P0-7）
- 檔案：strategies/f_mode.py:257-309
- 問題：gap-fill 不呼叫 `validate_pair()`、不檢查 `src.site != dest.site`、不檢查 `dest.site in transfer_sites`、手動修改 dict 而非使用 `apply_transfer()`。
- 修正：呼叫 `validate_pair()` 和 `apply_transfer()`；加入 src!=dest 檢查。

### NEW-D ND 模式緊急缺貨判斷忽略 Pending Received
- 檔案：strategies/nd_mode.py:52-58
- 問題：`is_no_stock = int(row['SaSa Net Stock']) == 0` 僅檢查淨庫存，應使用 total_available。
- 修正：改用 `total_available = net_stock + pending`；若 total_available > 0 則不視為緊急缺貨。

### NEW-E 精簡SKU ND 來源 1 件靜默丟棄
- 檔案：strategies/simplified_sku.py:109-111
- 問題：`pending_sources` 過濾 `transferable_qty >= 2`，ND 來源 1 件不配對也不退 D001。
- 修正：D001 退回階段同樣豁免 ND 1 件（與 RF 1 件一致）。

### NEW-F statistics.py Target 達成分析鍵不一致
- 檔案：services/statistics.py:111 vs 128
- 問題：recommendations 的 Receive Site 保持原始值，df 的 Site 做 `.strip().upper()`，同一站點可能被計為兩筆。
- 修正：統一使用 `.strip().upper()`。
