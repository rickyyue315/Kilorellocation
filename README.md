...
- **C1 模式新增 UI 選擇鍵**：可在側邊欄設定補0門檻（預設 N=1），調整 C1 模式處理的 total_available 上限。
- **C1 模式新增 UI 選擇鍵**：可在側邊欄設定補0門檻（預設 N=1），調整 C1 模式處理的 total_available 上限（原本僅處理 ≤1）。
...
| C1 | 重點補 0（只補 0/1，可自訂門檻） | 參照 C，僅補 total_available ≤ N（N 可於側邊欄設定），不回落一般缺貨 |
