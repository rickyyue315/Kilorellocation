...
- **D2 模式新增 UI 選擇鍵**：可切換「不限店舖數量（原有設定）」與「限制2間店舖接收（優化版）」。優化版限制每間 ND 轉出源最多配對 2 間 RF 接收店，並將接收量放大至 200%。
...
- **D2 模式新增 UI 選擇鍵**：可在側邊欄切換「不限店舖數量」或「限制2間店舖接收（優化版）」，優化版限制同一 SKU 各 ND 轉出源最多配對 2 間 RF 接收店，並將接收量放大至 200%。
