# KiLo 庫存調貨建議系統 v2.2.1

## 系統概述

KiLo 庫存調貨建議系統 v2.2.1 是一個基於 Streamlit 的智能庫存調配管理系統。專為零售運營設計，根據庫存、銷量、安全庫存和 MOQ 等數據，自動生成跨店鋪商品調貨建議，優化庫存分布並滿足銷售需求。

本系統採用**七模式設計**，靈活應對不同轉貨場景：
...
- ✅ **七模式系統**：A(保守轉貨) / B(加強轉貨) / B2(附加B特別模式) / C(重點補0) / D(清貨轉貨) / E(強制轉出) / F(目標優化)
...
- `Type`：B2 模式（附加B特別模式）必需，用於 Type=L 特殊轉出規則；Type=T 為遊客區店舖、Type=M 為混合型店舖
...
- 接收優先級分布（緊急缺貨 / 潛在缺貨 / 重點補0 / B2遊客區/混合型優先 / E模式接收 / F模式目標接收）
