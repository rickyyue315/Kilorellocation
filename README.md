# KiLo 庫存調貨建議系統 v2.2.2

## 系統概述

KiLo 庫存調貨建議系統 v2.2.2 是一個基於 Streamlit 的智能庫存調配管理系統。專為零售運營設計，根據庫存、銷量、安全庫存和 MOQ 等數據，自動生成跨店鋪商品調貨建議，優化庫存分布並滿足銷售需求。

本系統採用**八模式設計**，靈活應對不同轉貨場景：
...
- ✅ **八模式系統**：A(保守轉貨) / B(加強轉貨) / B2(附加B特別模式) / B3(附加B跨OM特別模式) / C(重點補0) / D(清貨轉貨) / E(強制轉出) / F(目標優化)
...
4. **模式選擇問題**
  - 確認選擇了正確的轉貨模式（A/B/B2/B3/C/D/E/F）
...
```
KiLo Reallocation/
├── app.py                          # Streamlit Web UI 主程序 (v2.2.2)
├── business_logic.py               # 調貨業務邏輯模組 (v2.2.2)
├── data_processor.py               # 數據預處理和驗證 (v2.2.2)
├── excel_generator.py              # Excel 報告生成器 (v2.2.2)
...
| 模組 | 功能 | 版本 | 依賴 |
|------|------|------|------|
| `app.py` | Streamlit UI、用戶交互、結果展示 | v2.2.2 | streamlit, pandas, matplotlib, seaborn |
| `business_logic.py` | 轉出/接收識別、優先級匹配、調貨算法 | v2.2.2 | pandas, numpy |
| `data_processor.py` | Excel 讀取、數據驗證、類型轉換、預處理 | v2.2.2 | pandas, openpyxl |
| `excel_generator.py` | Excel 報告生成、格式化輸出 | v2.2.2 | xlsxwriter, pandas |
