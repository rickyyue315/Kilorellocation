# KiLo 庫存調貨建議系統 v1.9.8

## 系統概述

KiLo 庫存調貨建議系統 v1.9.8 是一個基於 Streamlit 的智能庫存調配管理系統。專為零售運營設計，根據庫存、銷量、安全庫存和 MOQ 等數據，自動生成跨店鋪商品調貨建議，優化庫存分布並滿足銷售需求。

本系統採用**四模式設計**，靈活應對不同轉貨場景：
...
- ✅ **四模式系統**：A(保守轉貨) / B(加強轉貨) / C(重點補0) / D(清貨轉貨)
