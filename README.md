...
- B2/B2a/B3/B3a：若出貨店為 Type=M（Mix）且其總銷量高於目標店，該配對會被取消（`總銷量 = Last Month Sold Qty + MTD Sold Qty`）。
- B3/C2/E2/F 的跨 OM 規則：
- HD 來源不可轉到 HA/HB/HC。
- Windy 來源僅可轉到 Windy。
...
- 十三模式調貨策略。
- B2/B2a/B3/B3a 可在介面設定「單一出貨店舖最多配對接收店舖數」：`優先 1 間` / `最多 2 間` / `不限制`。
- B2/B2a/B3/B3a Mix 高銷量保護：避免高銷售 Mix 店舖被轉去較低銷售店舖。
...
- Python 3.8+
- 依賴：`pandas`、`openpyxl`、`streamlit`、`numpy`、`xlsxwriter`、`matplotlib`、`seaborn`、`ftfy`
...
| ND1 | ND 同 OM 轉貨 | ND 店舖可互轉（同 OM），按銷量智能排序，接收上限 2×兩月銷量 |
| ND2 | ND 混合 OM 轉貨 | ND 店舖可互轉（跨 OM），Windy 只轉 Windy，接收上限 2×兩月銷量 |

## 介面操作流程

1. 上傳 Excel。
2. 選擇模式。
3. 若為 B2/B2a/B3/B3a，可設定接收店數限制。
4. 點擊「生成調貨建議」。
5. 檢視表格與統計。
6. 下載 Excel 報表。
...
### B2/B2a/B3/B3a 的接收店數限制怎麼用？

- 在側欄選擇上述模式後，會出現「出貨店舖接收店數限制」。
