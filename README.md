...
- **可選 AI 批次 Notes**：按 Article 分組並行生成分析說明與風險標記，附加於每條建議（需 `AI_BATCH_NOTES_ENABLED=true`）。

## AI 功能說明（可選）

系統支援以下 AI 場景，均預設關閉（`AI_ENABLED=false`），不會影響既有調貨流程：

1. **AI 執行摘要**：調貨建議生成後，點擊「🤖 生成執行摘要」按鈕可獲得一段約 150 字的繁體中文重點摘要（整體規模、優先級分佈、需關注事項）。只傳聚合統計資料（總數、件數、優先級分佈），不傳 Article/Site/OM 明細。

2. **AI 批次 Notes**（需 `AI_BATCH_NOTES_ENABLED=true`）：在生成調貨建議後自動執行，按 Article 分組並行呼叫 OpenRouter API，為每條建議附加 30-50 字的繁體中文分析說明與風險標記（🔴高/🟡中/🟢低）。Notes 欄位中附加 `【AI分析: ...】` 和 `【AI風險: ...】` 內容，並新增 `AI Risk` 和 `AI Needs Review` 欄位。可在 UI 中檢視已分析條數。
...
$env:AI_ENABLED="true"
$env:AI_BATCH_NOTES_ENABLED="true"   # 可選，啟用 AI 批次 Notes
```
...
**AI 安全承諾**：AI 功能只提供可選摘要，不會修改確定性調貨結果；缺少 API key 或 AI 失敗時核心流程完整可用。優先級分組為純確定性規則，永遠 100% 可用。AI 批次 Notes 為附加資訊，不取代模板 Notes。
...
- `services/ai_batch_notes.py`：AI 批次 Notes 分析（可選、並行執行）
