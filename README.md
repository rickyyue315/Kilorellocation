# KiLo 庫存調貨建議系統 v2.4.0

## 系統概述

KiLo 庫存調貨建議系統 v2.4.0 是一個基於 Streamlit 的智能庫存調配管理系統。專為零售運營設計，根據庫存、銷量、安全庫存和 MOQ 等數據，自動生成跨店鋪商品調貨建議，優化庫存分布並滿足銷售需求。

本系統採用**十一模式設計**，靈活應對不同轉貨場景：
- **A 模式(保守轉貨)**：優先保護安全庫存，20% 轉出上限
- **B 模式(加強轉貨)**：積極處理滯銷品，50% 轉出上限
- **B2 模式(附加B特別模式)**：加入 Type 規則與接收優先排序，接收上限為安全庫存 2 倍
- **B3 模式(附加B跨OM特別模式)**：參照 B2，但允許跨 OM 配對（HD 不能轉到 HA/HB/HC；Windy 轉出只能到 Windy）
- **C 模式(重點補0)**：重點補充庫存為 0-1 的店鋪，30% 轉出上限
- **C2 模式(附加C跨OM重點補0)**：參照 C 模式邏輯，但允許跨 OM 配對（HD 不能轉到 HA/HB/HC；Windy 轉出只能到 Windy）
- **D 模式(清貨轉貨)**：針對 ND 店鋪無銷售記錄的清貨，特殊避免 1 件餘貨規則
- **E1 模式(強制轉出)**：針對標記為 *ALL* 的商品全數強制轉出，僅同OM配對
- **E1b 模式(強制轉出優先類型接收)**：針對標記為 *ALL* 的商品全數強制轉出，僅同OM配對，接收端優先Type=T/M
- **E2 模式(強制轉出跨OM)**：針對標記為 *ALL* 的商品全數強制轉出，優先同OM配對，可跨OM(HD除外)
- **F 模式(目標優化)**：Target 目標優先接收，其餘按補0邏輯

## 功能特點

- ✅ **十一模式系統**：A(保守轉貨) / B(加強轉貨) / B2(附加B特別模式) / B3(附加B跨OM特別模式) / C(重點補0) / C2(附加C跨OM重點補0) / D(清貨轉貨) / E1(強制轉出) / E1b(強制轉出優先類型接收) / E2(強制轉出跨OM) / F(目標優化)
...
- ✅ **接收優先級管理**：
  ...
  - 潛在缺貨補貨（庫存 < 安全庫存）
  - 重點補0（C/C2 模式，庫存 ≤ 1）
  - B2 接收優先級（遊客區/混合型店舖，先銷量後安全庫存）
  - B3/C2 跨OM配對（Windy 轉出限制，HD 目的地限制）
  - E 模式接收（E1/E1b/E2模式，RF店鋪可接收至Safety Stock × 2）
  - F 模式目標接收（優先滿足Target設定）
...
**模式專用欄位：**
- `ALL`：E1/E1b/E2 模式（強制轉出）必需，標記要強制轉出的商品行
- `Target`：F 模式（目標優化）必需，接收目標數量
- `Type`：B2/B3/E1b 模式必需，用於 Type=L 特殊轉出規則；Type=T 為遊客區店舖、Type=M 為混合型店舖；E1b模式接收端優先Type=T/M
...
| **B2** | 附加B特別模式 | 50% (RF) / 全轉出(Type=L且銷量≤2) | 特殊Type店鋪 | ND全轉出；Type=L且銷量≤2全轉出；銷量>2回到B模式；接收依遊客區/混合型優先 |
| **B3** | 附加B跨OM特別模式 | 50% (RF) / 全轉出(Type=L且銷量≤2) | 跨OM匹配 | 參照B2，但允許跨OM配對；HD不能轉到HA/HB/HC；Windy轉出只能到Windy |
| **C** | 重點補0 | 30% / 3 件上限 | 零庫存補貨 | 重點補充庫存 ≤ 1 的店鋪 |
| **C2** | 附加C跨OM重點補0 | 30% / 3 件上限 | 跨OM零庫存補貨 | 參照C模式，但允許跨OM配對；HD不能轉到HA/HB/HC；Windy轉出只能到Windy |
| **D** | 清貨轉貨 | N/A | ND 清貨 | ND 無銷售清貨，避免 1 件餘貨 |
| **E1** | 強制轉出 | 全數轉出 | 強制清貨(僅同OM) | 只處理標記 ALL 的商品行，僅同OM配對，接收上限為安全庫存 2 倍 |
| **E1b** | 強制轉出優先類型接收 | 全數轉出 | 強制清貨(僅同OM+優先類型) | 只處理標記 ALL 的商品行，僅同OM配對，接收端優先Type=T/M，接收上限為安全庫存 2 倍 |
| **E2** | 強制轉出跨OM | 全數轉出 | 強制清貨(可跨OM) | 只處理標記 ALL 的商品行，優先同OM配對，可跨OM(HD除外)，接收上限為安全庫存 2 倍 |
...
- 接收優先級分布（緊急缺貨 / 潛在缺貨 / 重點補0 / B2遊客區/混合型優先 / B3跨OM配對 / E模式接收(E1/E1b/E2) / F模式目標接收）
...
- B2 模式：RF 同 B 模式；Type=L 且銷量≤2 全數轉出（銷量>2回到B模式）；接收優先級：遊客區店舖(T)高銷量 → 混合型店舖(M)高銷量 → 遊客區店舖(T)高安全庫存 → 混合型店舖(M)高安全庫存
- B3 模式：參照 B2，但允許跨 OM 配對；HD 不能轉到 HA/HB/HC；Windy 轉出只能到 Windy
- C 模式：min(基礎可轉出, 庫存 × 30%, 最多 3 件)
- C2 模式：參照 C 模式，但允許跨 OM 配對；HD 不能轉到 HA/HB/HC；Windy 轉出只能到 Windy
...
### C2 模式(附加C跨OM重點補0) - 轉出規則

**轉出邏輯：完全沿用 C 模式**
- 優先級 1：ND 類型轉出（全數轉出）
- 優先級 2：RF 類型轉出（30% 上限 / 3 件上限）
...
- **Windy 限制**：Windy 轉出只能到 Windy 的店鋪，但 Windy 店鋪可接收其他 OM

**與 C 模式的差異：**
- C 模式：同 OM 內配對（分組按 Article + OM）
- C2 模式：跨 OM 配對（分組僅按 Article），但加入 HD/Windy 限制
...
4. **模式選擇問題**
  - 確認選擇了正確的轉貨模式（A/B/B2/B3/C/C2/D/E/F）
...
```
KiLo Reallocation/
├── app.py                          # Streamlit Web UI 主程序 (v2.3.0)
├── business_logic.py               # 調貨業務邏輯模組 (v2.3.0)
├── data_processor.py               # 數據預處理和驗證 (v2.3.0)
├── excel_generator.py              # Excel 報告生成器 (v2.3.0)
...
| 模組 | 功能 | 版本 | 依賴 |
|------|------|------|------|
| `app.py` | Streamlit UI、用戶交互、結果展示 | v2.3.0 | streamlit, pandas, matplotlib, seaborn |
| `business_logic.py` | 轉出/接收識別、優先級匹配、調貨算法 | v2.3.0 | pandas, numpy |
| `data_processor.py` | Excel 讀取、數據驗證、類型轉換、預處理 | v2.3.0 | pandas, openpyxl |
| `excel_generator.py` | Excel 報告生成、格式化輸出 | v2.3.0 | xlsxwriter, pandas |
