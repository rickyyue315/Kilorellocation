...
- **F/F2/F3 模式新增「優先滿足小型目標」選項**：勾選後 Priority-1 目標店按 needed_qty 升序排序（小型目標優先），最大化達成目標數量。
- **F/F2/F3 模式配對後缺口填補**：主要配對完成後，若有剩餘來源庫存，自動分配給尚未滿足的目標店。
...
- **C1 模式新增 UI 選擇鍵**：可在側邊欄設定補0門檻（預設 N=1）及每店轉出件數上限（3~10件，預設 3），調整 C1 模式處理範圍與源店分散程度。提高每店轉出上限可減少調出店舖總數量。
- **C1 模式新增 UI 選擇鍵**：可在側邊欄設定補0門檻（預設 N=1）及每店轉出件數上限（3~10件，預設 3），調整 C1 模式處理範圍與源店分散程度。提高每店轉出上限可減少調出店舖總數量（原本僅處理 ≤1，每店最多轉 3 件）。
...
- 工作表 3（F/F2/F3 模式）：`Target達成分析` — 每組 (Article, Site) 目標量與實際接收量對比、達成狀態、總達成率與總差距
