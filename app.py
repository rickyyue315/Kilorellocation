...
with st.sidebar:
    ...
    **核心功能：**  
    ...
    transfer_mode = st.radio(
        ...
        key='transfer_mode',
        help="A模式優先保障安全庫存，B模式則更積極地處理滯銷品，B2模式在B模式基礎上加入Type=L全轉出（若銷量>2則回到B模式），C模式重點補充庫存為0或1的店鋪，D模式針對ND店鋪無銷售記錄時的清貨處理，E模式強制轉出標記為*ALL*的商品，F模式使用Target數字優先滿足接收目標。"
    )
    ...
    with st.sidebar.expander("模式說明"):
        ...
        **轉貨模式：**
        ...
        - **B2模式(附加B特別模式)**：ND店鋪全轉出；Type=L在銷量≤2時全轉出(含RF)，若銷量>2則回到B模式；其餘RF依B模式規則
