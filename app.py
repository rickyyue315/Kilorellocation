...
with st.sidebar:
    ...
    **核心功能：**  
    ...
    2. **選擇轉貨模式**：在側邊欄選擇轉貨模式（A/B/B2/C/D/E/F）。
    ...
    transfer_mode = st.radio(
        ...
        key='transfer_mode',
        help="A模式優先保障安全庫存，B模式則更積極地處理滯銷品，B2模式在B模式基礎上加入Type=L全轉出（若銷量>2則回到B模式），並依Type T(遊客區)/Type M(混合型)的銷量與安全庫存排序接收優先級；C模式重點補充庫存為0或1的店鋪，D模式針對ND店鋪無銷售記錄時的清貨處理，E模式強制轉出標記為*ALL*的商品，F模式使用Target數字優先滿足接收目標。"
    )
...
if transfer_mode in ["A: 保守轉貨", "B: 加強轉貨", "C: 重點補0", "D: 清貨轉貨"]:
    ...
    - **Type 說明**：Type=T 為遊客區店舖、Type=M 為混合型店舖；B2接收優先級以此排序
    """)
