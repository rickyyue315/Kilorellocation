...
class TransferLogic:
    ...
    def _is_b3_family_mode(self, mode: str) -> bool:
        return mode in (self.mode_b3, self.mode_b3a, self.mode_b3l, self.mode_b3la)

    def _is_b_tourist_no_source_mode(self, mode: str) -> bool:
        return mode in (self.mode_b_special_a, self.mode_b2la, self.mode_b3a, self.mode_b3la)

    def _is_b_l_retain_mode(self, mode: str) -> bool:
        return mode in (self.mode_b2l, self.mode_b2la, self.mode_b3l, self.mode_b3la)
    ...
    def match_transfers(self, article: str, om: str, sources: List[Dict], 
        ...
        # 精簡SKU模式特殊處理
        if self._is_simplified_sku_mode(mode):
            return self._match_transfers_simplified_sku(sources, destinations, article, product_desc, mode)
    ...
    def _match_transfers_simplified_sku(self, sources: List[Dict], destinations: List[Dict],
                                        article: str, product_desc: str, mode: str) -> List[Dict]:
        """
        精簡SKU模式匹配邏輯：
        Phase 1: RF-to-RF配對（參照C1：最少2件起轉，優先可轉量較大來源）
        ...
        pending_sources = sorted(
            ...
            key=lambda x: (-x['priority'], -int(x.get('transferable_qty', 0)))
        )
        ...
        for source in pending_sources:
            ...
            for dest in temp_destinations:
                ...
                if cross_om and self._is_hd_to_hk_restricted(source.get('site', ''), dest.get('site', '')):
                    continue
    ...
    def _match_transfers_f_mode(self, sources: List[Dict], destinations: List[Dict],
        ...
        5. 總轉移量不超過總需求量
        6. F2模式僅允許Target店舖接收（由identify_destinations控管）
        """
        recommendations = []
        ...
        def _f_source_sort_key(src, dest_om, dest_site=''):
            ...
            if rp == 'ND':
                ...
                tier = 2
            hd_penalty = 0
            if (mode == self.mode_f_target_only and self.f2_allow_hd_transfer
        ...
        for priority_level in [1, 2]:
            ...
            for dest in priority_dests:
                ...
                # F/F2模式：轉出優先級 ND > 同OM RF > 其他OM RF > HD(僅F2+允許HD轉出)
                if mode in (self.mode_f, self.mode_f_target_only):
                ...
                for source in sorted_sources:
                    ...
                    # F2模式例外：若啟用f2_allow_hd_transfer，則允許HD轉出（排序已降為最低優先）
                    if self._is_hd_to_hk_restricted(source['site'], dest['site']):
                    ...
                    if dest.get('target_qty') is not None and received_qty_by_site[receive_site_key] >= dest['target_qty']:
                        # dest['needed_qty'] 已在之前的每次轉移中逐步扣除
                        # 若因單次轉移量大於剩餘 needed_qty 而導致 needed_qty 變為負值，
                        # 則之前扣除的 transfer_qty 已大於實際 needed_qty，remaining_demand 已多扣
                        # 此處將 needed_qty 歸零即可，不再額外扣除，否則會重複計算
                        dest['needed_qty'] = 0
...
# HD限制檢查：HD店鋪不能轉去HA/HB/HC店鋪
    ...
    def _match_by_priority(self, sources: List[Dict], destinations: List[Dict], 
        ...
        for source in filtered_sources:
            ...
            for dest in filtered_destinations:
                ...
                # D/D2模式：檢查累計接收上限（不超過Safety Stock）
    ...
    def generate_transfer_recommendations(self, df: pd.DataFrame, mode: str) -> List[Dict]:
        ...
        # 驗證模式
        if mode not in [self.mode_a, self.mode_b, self.mode_b_special, self.mode_b_special_a, self.mode_b2l, self.mode_b2la, self.mode_b3, self.mode_b3a, self.mode_b3l, self.mode_b3la, self.mode_c, self.mode_c1, self.mode_c2, self.mode_d, self.mode_d2, self.mode_e1, self.mode_e1b, self.mode_e2, self.mode_f, self.mode_f_target_only, self.mode_nd1, self.mode_nd2, self.mode_simplified_sku_same, self.mode_simplified_sku_cross]:
        ...
        # 根據模式選擇分組方式
        if mode in [self.mode_e2, self.mode_f, self.mode_f_target_only, self.mode_b3, self.mode_b3a, self.mode_b3l, self.mode_b3la, self.mode_c2, self.mode_nd2]:
            # E2/F/F2/B3/B3L/C2/ND2模式支持跨OM配對，因此僅按Article分組
        ...
        for group_keys, group_df in grouped:
            ...
            # E1/E1b/E2/F/F2/B2/B2a/B2L/B2La/B3/B3a/B3L/B3La/C2/ND1/ND2/精簡SKU模式特殊處理：從destinations中過濾掉同時作為轉出源的店鋪
            if mode in [self.mode_e1, self.mode_e1b, self.mode_e2, self.mode_f, self.mode_f_target_only, self.mode_b_special, self.mode_b_special_a, self.mode_b2l, self.mode_b2la, self.mode_b3, self.mode_b3a, self.mode_b3l, self.mode_b3la, self.mode_c2, self.mode_nd1, self.mode_nd2, self.mode_simplified_sku_same, self.mode_simplified_sku_cross]:
                source_sites = set([s['site'] for s in sources])
                destinations = [d for d in destinations if d['site'] not in source_sites]
            
            # 執行匹配
            if mode in [self.mode_e2, self.mode_f, self.mode_f_target_only, self.mode_b3, self.mode_b3a, self.mode_b3l, self.mode_b3la, self.mode_c2, self.mode_nd2, self.mode_simplified_sku_cross]:
    ...
    def perform_quality_checks(self, df: pd.DataFrame, mode: str = '') -> bool:
        ...
        if not self._is_nd_transfer_mode(mode) and mode not in (self.mode_f, self.mode_f_target_only):
            for rec in self.transfer_recommendations:
    ...
    def _create_recommendation_note(self, source: Dict, dest: Dict, current_received_qty: int, transfer_qty: int, mode: str) -> str:
        ...
        if self._is_simplified_sku_mode(mode):
            if dest.get('dest_type') == '退回D001':
                ...
                mode_variant = "限同OM" if mode == self.mode_simplified_sku_same else "跨OM"
